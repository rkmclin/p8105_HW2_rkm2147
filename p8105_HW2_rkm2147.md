...
bottles so far is 6.4436510^{5}. The median number of sports balls in
2017 was 8. There is a total of 24 rows in the data frame. The total
amount of precipitation in 2018 was 70.33 inches.
...
route\_number. In additions to the route numbers, entry and entrance
type, vending, and ADA. Cleaning of the data so far consisted of
dropping tangential variables, cleaning up variable names using the
janitor tool, and changing entry into a logical variable. The size of
the resulting data set is 20548 by 10.

There are 465 distinct stations within the NYC transit. There are a
total of 84 distinct stations that are ADA compliant.

There are a total of 60 distinct stations that serve the A train. The
proportion of station entrances / exits without vending that allow
entrance is 0.1043925. There are a total of 17 distinct stations that
serve the A train that are ADA compliant.
